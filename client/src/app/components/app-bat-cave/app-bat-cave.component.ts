...
export class AppBatCaveComponent implements AfterViewInit, OnDestroy {
  ...
  public loadBatcaveModel(): void {
    ...
    dracoLoader.setDecoderPath(APP_CONFIG.DRACO_DECODER_PATH);
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        this.loadingService.updateInitialProgress(100, 'Loading 3D Model');
      ...
      (progress) => {
        // Handle edge cases where progress.total might be 0 or undefined
        let percentComplete = 0;
        if (progress.total && progress.total > 0) {
          percentComplete = Math.min(100, (progress.loaded / progress.total) * 100);
        } else if (progress.loaded > 0) {
          // If total is unknown but we have loaded data, show some progress
          percentComplete = Math.min(90, progress.loaded / 1000); // Estimate based on loaded bytes
        }
        ...
        if (currentQuarter > this.lastLoggedProgress && currentQuarter >= 25) {
          console.debug(`Loading progress: ${percentComplete.toFixed(2)}% (${progress.loaded}/${progress.total || 'unknown'})`);
